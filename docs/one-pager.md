...
- **Four-class taxonomy.** Where click models treat outcomes as binary (clicked / not clicked), episode segmentation reveals four behaviorally distinct cursor outcomes per result: **clicked**, **deferred** (cursor entered, retreated, returned later), **evaluated-rejected** (cursor entered and didn't return), and **not-approached** (cursor never entered the AOI). All four are recoverable from cursor telemetry alone (no eye tracker). NB22 click prediction AUC 0.792 with no element-type features (post coordinate-space audit 2026-04-09; was 0.821), element-type interaction AUC pending re-verification.
