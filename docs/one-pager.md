...
- **Four-class taxonomy.** Where click models treat outcomes as binary (clicked / not clicked), episode segmentation reveals four behaviorally distinct cursor outcomes per result: **clicked**, **deferred** (cursor entered, retreated, returned later), **evaluated-rejected** (cursor entered and didn't return), and **not-approached** (cursor never entered the AOI). All four are recoverable from cursor telemetry alone (no eye tracker). NB22 click prediction AUC 0.821 with no element-type features, 0.838 with element-type interactions.
...
1. **The four-class taxonomy is invisible to binary click models.** Splitting "not clicked" into deferred, evaluated-rejected, and not-approached requires episode segmentation. A Transformer trained on click prediction collapses the three negative classes into one and loses the deferred signal entirely. The deferred class is the one that matters for understanding how users come back to results — and it is the one that adaptive ranking systems should care about.
