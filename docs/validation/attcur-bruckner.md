...
> **Regime: [WILD, ACD]** — every number and claim in this document is computed on the Attentive Cursor Dataset (Leiva & Arapakis 2020), which has *cursor and click only, no eye tracker, no pupil*. The LAB counterparts (AdSERP: pupil + gaze + cursor) live in the upstream `attentional-foraging` repo and are tagged `[LAB, NB##:K##]` there. The whole point of this document is to test what survives when you strip the eye tracker — see `CLAUDE.md` for the LAB ↔ WILD bridge convention.
