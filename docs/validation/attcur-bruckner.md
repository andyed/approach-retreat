...
> **Regime: [WILD, ACD]** — every number and claim in this document is computed on the Attentive Cursor Dataset (Leiva & Arapakis 2020), which has *cursor and click only, no eye tracker, no pupil*. The LAB counterparts (AdSERP: pupil + gaze + cursor) live in the upstream `attentional-foraging` repo and are tagged `[LAB, NB##:K##]` there. The whole point of this document is to test what survives when you strip the eye tracker — see `CLAUDE.md` § "Role in the CIKM paper: the LAB ↔ WILD bridge" for the convention.
...
- **The four-class deferred-vs-evaluated-rejected taxonomy** — that finding is `[LAB, NB22:K1–K7]`, from AdSERP (multi-AOI, 150 Hz eye tracker + cursor) and is documented in [`docs/theory.md`](../theory.md). The `[WILD, ACD]` single-AOI, 1-Hz structure cannot separate the two non-click sub-populations — the four-class taxonomy is a LAB-only finding until a multi-AOI in-the-wild dataset becomes available.
