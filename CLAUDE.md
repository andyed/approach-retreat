...
- **Viewport bands (2026-04-19, v0.2.0):** per-AOI cumulative ms in `{any, top, mid, bot}` viewport bands, piecewise-constant over scroll/resize/reflow. JS `computeViewportBandsPure` parity-tested against `viewport_ms_for_trial` in the calibration script (Δ = 0). Calibration (LAB, n = 2,351, bootstrap 95 % CIs): retreat-alone 0.796 [0.759, 0.830], bands-alone 0.800 [0.774, 0.828], combined **0.842 [0.818, 0.864]**. Fully-contextual viewport (all 10 AOIs × rank dummies) is worse — the signal is local per-AOI. `vt_top` coefficient is rank-dependent over **P0–P5 only**: +2.02 at P0, crosses 0 at P5 (the significance transition). Positions past P5 are not reported as a headline — too sparse (n ≤ 91 per slice), class-balance-inverted, and participant-concentrated (top 4 drive most of the signal). Consumers apply per-rank weights, library emits raw ms. Tag: `edmonds-2026-vpbands-v1`. See `docs/validation/viewport-bands-calibration.md`.
