...
def write_curation_md(curation: dict, n_pool: int) -> None:
    ...
    lines = [
        ...
        "",
        f"2. **Pass 2** (this script) takes the top **{n_pool}** with a per-participant cap, extracts organic AOI bboxes, builds replay bundles with derived labels, and assigns each trial to the most diagnostic group it qualifies for (priority order: E → B-DEF → B-REJ → C → D → A).",
        "",
        "## Groups",
        "",
    ]
