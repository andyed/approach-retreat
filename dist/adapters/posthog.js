...
function buildTrajectory(samples, stride) {
  ...
  for (let i = 0; i < samples.length; i += stride) {
    ...
    flat.push(
      ...
      Math.round(s.vx * 1e3) / 1e3,
      Math.round(s.vy * 1e3) / 1e3
    );
  }
  return flat;
}
function extractTargetFields(el, pathDepth = 3) {
  ...
  out.target_name = out.target_label || out.target_aria_label || out.target_id || out.target_text || out.target_title || out.target_tag;
...
function selectorFor(el) {
  ...
  const useful = classList.filter((c) => c && /^[a-zA-Z_][\w-]*$/.test(c)).slice(0, 2);
  ...
  if (parent) {
    ...
    if (!sawSibling) {
      for (let i = 0; i < parent.children.length; i++) {
        const sib = parent.children[i];
        if (sib !== el && sib.tagName === el.tagName) {
          sawSibling = true;
          break;
        }
      }
...
function createPostHogAdapter(posthog, options = {}) {
  ...
  const adapter = {
    ...
    captureSummary(ar) {
      ...
      const timeToFirstClickMs = firstClick && sessionStart != null ? Math.round(firstClick.clicked_at - sessionStart) : null;
      ...
      const approachFeatures = typeof ar.getApproachFeatures === "function" ? ar.getApproachFeatures() : [];
      const viewportBands = typeof ar.getViewportBands === "function" ? ar.getViewportBands() : [];
      const bandByPosition = new Map(viewportBands.map((b) => [b.position, b]));
      const viewportAnalytics = typeof ar.getViewportAnalytics === "function" ? ar.getViewportAnalytics() : [];
      ...
      const bandContext = typeof ar.getViewportBandContext === "function" ? ar.getViewportBandContext() : { viewport_h: null, schema: null };
      const analyticsContext = typeof ar.getViewportAnalyticsContext === "function" ? ar.getViewportAnalyticsContext() : { viewport_h: null, viewport_center_tol_px: null, schema: null };
