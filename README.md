...
On AdSERP-LAB against the gaze-derived deferred-vs-evaluated-rejected label `[LAB, NB30/NB28]`: approach-retreat M4 alone, banded viewport residence alone, and retreat + bands combined are *[bbox-organic re-calibration pending; multi-hour bootstrap × 47-fold StratifiedGroupKFold gates the post-cascade values]*. Pre-cascade calibration (legacy absolute-attribution): M4 alone 0.796 [0.759, 0.830]; bands-alone 0.800 [0.774, 0.828]; retreat + bands combined 0.842 [0.818, 0.864] (source: [`docs/validation/viewport-bands-calibration.md`](docs/validation/viewport-bands-calibration.md)). The NB30 six-feature B∪C′ minimal set construct (continuous viewport residence + `min_abs_velocity` + `n_reversals`) and the bands-add-no-classifier-AUC finding (NB30:K14) carry through structurally; the specific bbox-cascade AUC numbers land when NB28 retrains. Mobile / feed / grid transfer is capability-claimed but untested; cross-surface validation is future work.
